...
export default function Hero() {
  ...
  const stack = [
    ...
    { label: t('hero.stackMore'), color: '#8a8aaa', bg: 'rgba(107,107,138,0.07)' },
  ]

  const extras = t('hero.extras').map((label, i) => {
    const colors = [
      { color: '#f5ff00', bg: 'rgba(245,255,0,0.07)' },
      { color: '#b44fff', bg: 'rgba(180,79,255,0.07)' },
      { color: '#00ffe5', bg: 'rgba(0,255,229,0.07)' },
    ]
    return { label, ...colors[i] }
  })

  const badgePhrases = t('hero.badges')
  ...
  return (
    <section id="hero" className="hero-grid grid relative overflow-hidden"
      ...
      <div className="relative z-10">
        <span className="hero-badge" style={{ animation: 'fadeInUp 0.6s ease both', marginBottom: '1.5rem' }}>
      ...
      <div className="hero-terminal relative z-10 flex justify-end" style={{ animation: 'fadeInUp 0.6s 0.5s ease both', marginTop: 40 }}>
        <div className="overflow-hidden" style={{
          ...
          <div className="flex items-center gap-2 px-4 py-3" style={{ background: '#1a1a26', borderBottom: '1px solid rgba(0,255,229,0.15)' }}>
            ...
            <span className="font-mono text-xs ml-2" style={{ color: '#8a8aaa' }}>raquel@lapencadev ~</span>
          </div>
          ...
          <div className="p-6 font-mono text-sm leading-relaxed">
            ...
            <div className="pl-4" style={{ color: '#8a8aaa' }}>{'{'}</div>

            {[
              ['"name"',       '"Raquel Sánchez Guirado"'],
              ['"role"',       '"Software Engineer"'],
              ['"background"', `"${t('hero.terminalBackground')}"`],
            ].map(([k, v]) => (
              <div key={k} className="pl-4">
                <span style={{ color: 'var(--neon-yellow)' }}>{k}</span>
                <span style={{ color: '#8a8aaa' }}>: </span>
            ...
            <div className="pl-4 mt-2">
              ...
              <span style={{ color: '#8a8aaa' }}>: </span>
              <span style={{ color: '#a8a8c0' }}>"{t('hero.terminalMotto')}"</span>
            </div>

            <div className="pl-4 mt-1" style={{ color: '#8a8aaa' }}>{'}'}</div>
